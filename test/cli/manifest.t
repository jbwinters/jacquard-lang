...
taints every caller's row.
  ...
  > EOF_WFT
  $ jacquard check hostile.wft --print-sigs
...
the effect and the call-chain endpoint:
  ...
  $ jacquard check hostile.wft --manifest console
...
handler:
  ...
  $ jacquard check hostile.wft --manifest net,console
  ok
  $ jacquard run hostile.wft --allow net
...
that would only bounce with E0703:
  ...
  $ cat > pure.wft <<'JACQUARD'
  > (app (var option.get!) (var none))
  > JACQUARD
  $ jacquard run pure.wft
  ...
  [3]
  $ jacquard run pure.wft --allow abort
