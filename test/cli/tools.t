...
The same script under real grants performs the writes:
  ...
  $ weft run script.wft --allow fs --allow net > /dev/null
  $ cat report.txt
  hello
...
observation, not consequence).
  ...
  $ weft run reader.wft --dry-run | head -1
  "fixture-content"
...
what renders the request divergence.
  ...
  >           (clause (pcon mk-response (pwild) (pvar b)) (var b)))))))
  > WEFT
  $ weft replay trace.wft prog.wft
  "beta."
  $ weft replay trace.wft prog.wft --fork '1=(response 500 "down")'
  "beta."
  $ weft replay trace.wft prog.wft --fork '1=(response 500 "down")' --compare
  "beta."
...
to the live-dry world so the futures are distinguishable):
  ...
  $ weft replay trace.wft prog.wft --fork '1=(response 200 "up")' --to 1 | tail -1
  "<live-after-log>"
  $ weft replay trace.wft prog.wft --fork '1=(response 503 "down")' --to 1 | tail -1
  "<live-after-log>"
  $ weft replay trace.wft prog.wft --fork '1=(response 200 "up")' --to 1 --compare | grep op2
    op2: identical request
  $ weft replay trace.wft prog.wft --fork '1=(response 503 "down")' --to 1 --compare | grep -c fallback
  1

Scrubbing: --to N serves the log through op N, then the dry world takes over.

  $ weft replay trace.wft prog.wft --to 1
  "<live-after-log>"
...
P(0)=0.16, P(1)=0.48.
  ...
  $ weft dist-diff coins-a.wft coins-b.wft --tolerance 0.001 2>/dev/null
  P(2): 0.250000 -> 0.360000 (delta +0.110000)
  P(0): 0.250000 -> 0.160000 (delta -0.090000)
  P(1): 0.500000 -> 0.480000 (delta -0.020000)
  $ weft dist-diff coins-a.wft coins-a.wft 2>/dev/null
  no divergence
...
addressing, not bookkeeping).
  ...
  $ weft dist-diff coins-a.wft coins-b.wft --tolerance 0.001 2>&1 >/dev/null | grep -c cached
  2
  $ weft dist-diff coins-a.wft coins-b.wft --tolerance 0.001 --sweep prior=0.5,0.9 2>/dev/null
  -- sweep prior=0.5 --
  no divergence
  -- sweep prior=0.9 --
...
review story: "this change adds net, authored by agent X" — read first).
  ...
  $ weft check authored.wft --print-sigs --origin agent:weft-demo-5
  summarizer : (text) ->{net} text [agent:weft-demo-5]

TL.3: malformed --fork specs refuse instead of silently running the baseline.

  $ weft replay trace.wft prog.wft --fork 'garbage'
