...
mutates.
  ...
  > JACQUARD
  $ jacquard run script.wft --dry-run
  "<dry-run response>"
...
The same script under real grants performs the writes:
  ...
  $ jacquard run script.wft --allow fs --allow net > /dev/null
  $ cat report.txt
  hello
...
observation, not consequence).
  ...
  $ jacquard run reader.wft --dry-run | head -1
  "fixture-content"
...
The log is a readable form; the program is any .wft; forked futures run under
...
what renders the request divergence.
  ...
  >           (clause (pcon mk-response (pwild) (pvar b)) (var b)))))))
  > JACQUARD
  $ jacquard replay trace.wft prog.wft
  "beta."
  $ jacquard replay trace.wft prog.wft --fork '1=(response 500 "down")'
  "beta."
  $ jacquard replay trace.wft prog.wft --fork '1=(response 500 "down")' --compare
  "beta."
...
to the live-dry world so the futures are distinguishable):
  ...
  $ jacquard replay trace.wft prog.wft --fork '1=(response 200 "up")' --to 1 | tail -1
  "<live-after-log>"
  $ jacquard replay trace.wft prog.wft --fork '1=(response 503 "down")' --to 1 | tail -1
  "<live-after-log>"
  $ jacquard replay trace.wft prog.wft --fork '1=(response 200 "up")' --to 1 --compare | grep op2
    op2: identical request
  $ jacquard replay trace.wft prog.wft --fork '1=(response 503 "down")' --to 1 --compare | grep -c fallback
  1

Scrubbing: --to N serves the log through op N, then the dry world takes over.

  $ jacquard replay trace.wft prog.wft --to 1
  "<live-after-log>"
...
P(0)=0.16, P(1)=0.48.
  ...
  >       (clause (pwild) (lit 1)))))
  > JACQUARD
  $ sed 's/(lit 0.5)/(lit 0.6)/' coins-a.wft > coins-b.wft
  $ jacquard dist-diff coins-a.wft coins-b.wft --tolerance 0.001 2>/dev/null
  P(2): 0.250000 -> 0.360000 (delta +0.110000)
  P(0): 0.250000 -> 0.160000 (delta -0.090000)
  P(1): 0.500000 -> 0.480000 (delta -0.020000)
  $ jacquard dist-diff coins-a.wft coins-a.wft 2>/dev/null
  no divergence
...
RESULT TYPES differ refuse before any probability is compared:
  ...
  $ cat > coin-support.wft <<'JACQUARD'
  > (match (app (var sample) (app (var bernoulli) (lit 0.5)))
  >   (clause (pcon true) (lit 1))
  >   (clause (pcon false) (lit 3)))
  > JACQUARD
  $ jacquard dist-diff coins-a.wft coin-support.wft --tolerance 0.001 2>/dev/null | sort
  support gained: 3
  support lost:   0
  support lost:   2
  $ printf '(match (app (var sample) (app (var bernoulli) (lit 0.5))) (clause (pcon true) (lit "yes")) (clause (pcon false) (lit "no")))\n' > texty.wft
  $ jacquard dist-diff coins-a.wft texty.wft 2>&1 | head -1
  error[E0801]: dist-diff: model result types differ (coins-a.wft : int, texty.wft : text); probabilities over different types are not comparable
...
addressing, not bookkeeping).
  ...
  $ jacquard dist-diff coins-a.wft coins-b.wft --tolerance 0.001 2>&1 >/dev/null | grep -c cached
  2
  $ jacquard dist-diff coins-a.wft coins-b.wft --tolerance 0.001 --sweep prior=0.5,0.9 2>/dev/null
  -- sweep prior=0.5 --
  no divergence
  -- sweep prior=0.9 --
