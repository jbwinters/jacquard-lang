...
corpus goldens; here we check the shape and determinism.
  ...
  > (lit 2)
  > EOF
  $ jacquard hash one.wft | sed 's/[0-9a-f]\{64\}/HASH/'
  0 HASH
  0:one HASH
  1 HASH
...
Determinism: two runs agree byte for byte.
  ...
  $ jacquard hash one.wft > a.txt && jacquard hash one.wft > b.txt && cmp a.txt b.txt

Parse failures exit 1:

  $ printf '(' > bad.wft
  $ jacquard hash bad.wft
