...
Exact enumeration of the two-coins model (2/3, 1/3):
  ...
  $ jacquard hash ../../demos/m3-two-coins.wft > model.before.hash
  $ sed 's/[0-9a-f]\{64\}/HASH/' model.before.hash
  0 HASH
  $ jacquard infer enumerate ../../demos/m3-two-coins.wft
  0.666667  true
  0.333333  false
  $ jacquard hash ../../demos/m3-two-coins.wft > model.after-enum.hash
...
Likelihood weighting with a fixed seed converges and is reproducible:
  ...
  0.667898  true
  0.332102  false
  $ jacquard hash ../../demos/m3-two-coins.wft > model.after-lw.hash
...
A different seed differs:
  ...
  $ jacquard infer lw ../../demos/m3-two-coins.wft --seed 7 --samples 1000 > a.txt
  $ jacquard infer lw ../../demos/m3-two-coins.wft --seed 8 --samples 1000 > b.txt
  $ cmp -s a.txt b.txt; echo "same=$?"
  same=1

The seed is required (decision D4):

  $ jacquard infer lw ../../demos/m3-two-coins.wft 2>&1 | grep -c 'required'
  1
...
An ungranted effect inside a model still refuses (dist itself is implicit):
  ...
  > EOF_WFT
  $ jacquard infer enumerate naughty.wft
