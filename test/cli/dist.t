...
value; the seed makes runs reproducible.
  ...
  $ cat > die.wft <<'JACQUARD'
  > (app (var sample) (app (var uniform-int) (lit 1) (lit 6)))
  > JACQUARD

  $ jacquard run die.wft --allow dist --seed 42
  5
  $ jacquard run die.wft --allow dist --seed 42
  5
  $ jacquard run die.wft --allow dist --seed 7
  3

Without the grant, the manifest refuses to start (exit 3):

  $ jacquard run die.wft
...
run cannot condition, only jacquard infer can.
  ...
  $ cat > obs.wft <<'JACQUARD'
  > (app (var observe) (app (var bernoulli) (lit 0.5)) (var true))
  > JACQUARD

  $ jacquard run obs.wft --allow dist --seed 1
...
handler (the M3 thesis):
  ...
  $ jacquard infer enumerate die.wft
  0.166667  1
  0.166667  2
  0.166667  3
  0.166667  4
