...
type record
(** One validated [audit-chain-v2] record. Its digest commits to the predecessor and the canonical
    bytes of its embedded sequenced [audit-entry-v2] form. *)

val genesis : Hash.t
(** The fixed predecessor for an empty v2 chain. *)

val digest : previous:Hash.t -> Form.t -> (Hash.t, Diag.t list) result
(** [digest ~previous entry] validates [entry] against the released v2 AuditEntry wire schema and
    exact versioned GovernanceAssessment/GovernanceOutcomeSummary carriers, then hashes its existing
    {!Printer.print_compact} bytes with [previous]. Malformed entries return E1302. *)
...
(** [verify_string ~file ~expected_head bytes] strictly verifies a canonical v2 chain. Every record
    must occupy one LF-terminated canonical line. Contiguous zero-based sequences, linkage, embedded
    entry schema, stored digests, and the published [expected_head] are checked in order. Empty
    input verifies only against {!genesis}. All failures are diagnostics (E1301--E1308); malformed
    input never raises. *)
...
(** [append_file ~file ~previous entry] first verifies the existing file against the caller's
    ...
    operation remains intended for a single cooperating writer. A negative entry sequence returns
    E1308 before opening or writing the file; I/O or concurrent-change failures return E1306 and no
    exception escapes. *)
