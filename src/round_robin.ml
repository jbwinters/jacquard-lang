  ...
  if bounds.max_tasks <= 0 then Error (Runtime_err.Scheduler_error "task bound must be positive")
    ...
    let rec drive () =
      ...
      | runnable ->
          ...
          in
          let rec take_chosen prefix = function
            | [] -> Error [ Diag.error ~code:"E0908" "chosen replay task is not runnable" ]
            | ((run, handle) as candidate) :: rest ->
                let* candidate_id = id run handle in
                if Concurrency_contract.compare_task_id candidate_id chosen_id = 0 then
                  Ok (candidate, List.rev_append prefix rest)
                else take_chosen (candidate :: prefix) rest
          in
