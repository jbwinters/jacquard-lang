...
One product-shaped walk through what makes Weft unusual, using only shipped
machinery (release 0.1; the full transcript is pinned in `test/cli/escrow.t`):

1. `workflow.wft` is a generated-looking publish workflow.
2. `weft check --print-sigs` prints its signature — the row IS its manifest:
...
10. `workflow-escalated.wft` requests authority beyond the approved manifest
