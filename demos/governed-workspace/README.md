...
- a host-bridge denial through the real `Governance_approval_bridge` and
  durable `Governance_approval_queue`, with the existing test asserting exact
  proposal binding and zero driver calls;
...
The script builds the two existing executables it needs. `agent.jac` is the
readable public application. `story.jac` contains deterministic interpreters,
`tests.jac` contains sampled and exhaustive Warp laws, and the three `.jqd`
files are audit-chain evidence fixtures—not hand-maintained program twins.
