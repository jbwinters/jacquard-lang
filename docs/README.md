...
- `concurrency.md`: SC.4 generalized child-effect charging and the SC.5
  policy-independent lifecycle core over the SC.3 Task/Async boundary, plus
  scope contracts, deterministic scheduling, cancellation, exclusions, and
  D46-D50.
...
- `release/structured-concurrency/EVIDENCE.md`: combined SC.4 generalized row-law
  and SC.5 scheduler-lifecycle evidence over the SC.3 Task value,
  private-carrier, Async declaration, and native parity boundary.
- `release/structured-concurrency/MANIFEST.sha256`: reconstructible combined
  SC.4 + SC.5 overlay on the validated SC.3 base.
