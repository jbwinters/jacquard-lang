...
record; D36 completed in SX.27 as described below. Any later revision must record and review
...
grammar. D36 generated accessors and declaration-time label validation
completed in SX.27. The later explicit-dictionary decision ratifies ordinary `Eq`,
...
is a fresh binding pattern. Labeled-field declarations generate one accessor
per label that every constructor carries (D36, SX.27), named after the type:

```jacquard
type Fleet = | MkFleet(inv: SvcMood, pay: SvcMood, shp: SvcMood)
-- generates fleet.inv, fleet.pay, and fleet.shp; fleet.inv(MkFleet(a, b, c)) returns a
```
...
SX.27 completes D36. Lowering follows each labeled type declaration with one
ordinary pure definition `<type-kebab>.<label>(value)` for every label that all
of the type's constructors carry; it matches each constructor and returns that
field, so the accessor is total. A label that only some constructors carry
keeps its pattern and named-construction uses but has no accessor, because a
pure accessor must answer for every constructor. Labels are validated where the
type is declared: a label repeated within one constructor is E1239, a label
...
Generated constructor accessors and declaration-time label validation ship
(D36, SX.27). Labeled declarations, generated accessors, and partial
...
not cover D36 accessor generation and label validation, which SX.27 later
completed, or the parked resource-scoped-row idea, and it does not freeze future surface revisions.
