# Jacquard Surface Syntax — Design, Draft 0.2
...
Draft 0.2 is the frozen v0 implementation contract. Decisions D27-D40 in
section 10 are no longer open design questions for the surface-syntax slice.
An implementation may discover a correctness problem, but it must record and
review any resulting decision change rather than quietly choosing different
syntax.
...
and the status of laws L1-L7 before `.jac` is advertised as stable.
...
is a fresh binding pattern. Labeled fields generate ordinary accessors:

```jacquard
type Fleet = | MkFleet(inv: SvcMood, pay: SvcMood, shp: SvcMood)
-- generated accessor: fleet.inv(fleet)
```
...
labeled (D36): `field: Type` inside a constructor's parens, one accessor
function generated per label at zero extra kernel cost (docs/ast.md §5.3),
so `fleet.inv` is an ordinary dotted call once the type is declared, not new
syntax. Both field exercises invented this exact `Ctor(field: Type, ...)`
notation independently. Labeled patterns are deferred past v0 (§5, Match).
...
A field label generates `<type-kebab>.<label>` as an ordinary pure function.
For v0, a generated accessor is legal only when that label occurs exactly once
in every constructor of the type and has the same field type in each; this
makes the generated match exhaustive and its result type unambiguous. A label
that is missing, duplicated, type-inconsistent, or collides with an explicit
term is a diagnostic. Generated accessors carry `surface-generated` metadata
and are emitted as ordinary `DefTerm` declarations after the `DefType`; the
metadata lets the surface printer avoid printing the generated body twice.
...
Labeled constructor *patterns* (D36 defers them; declarations and accessors
ship now). Interpolation sugar for text (D38 logs a prelude-only `text.join`
for now; a real syntax form is a separate design pass against L4).
...
- **S0 (SS.0-SS.4, tasks 87-91), printer and spec, M.** Freeze the
  ...
  trees, this grammar frozen against it, L1 and L7 tested. No parser yet;
...
| D36 | labeled fields | `Ctor(field: Type, ...)` declarations plus generated accessors ship in v0; labeled patterns deferred; positional matches past four fields are linted |
