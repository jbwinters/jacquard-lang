...
\textbf{Status: historical initial design document.}
This paper records the thesis and architecture that preceded the
implementation. Its roadmap, implementation status, surface-syntax
discussion, and some proposed probabilistic features are outdated. For
current behavior and limits, see \texttt{README.md},
\texttt{docs/release/0.1/LIMITS.md},
\texttt{docs/release/0.1/CLAIMS.md}, and
\texttt{docs/surface-syntax.md}.
\end{minipage}}
\end{center}
...
Evaluation is strict and left to right; Haskell's space-leak lesson argues that predictability wins for machine-written code, and laziness on demand is a zero-arity lambda whose type honestly displays its row. Handlers are deep. This initial design made resumptions multi-shot; the implemented successor retains reusable continuations for explicit \texttt{Multi} operations and gives resource-bearing \texttt{Once} operations affine resumptions. Section~\ref{sec:dist} explains why multi-shot remains non-negotiable for search. Matches are exhaustive, failure is an effect with a row entry, and no ambient handlers exist.

One implementation note is owed here as an honest correction of an earlier plan. OCaml~5's native effect handlers are one-shot: resuming a continuation twice raises. Since exact inference by enumeration requires resuming a captured continuation once per element of a distribution's support, the reference interpreter cannot lean on OCaml's native handlers and instead uses continuation-passing style. In the implemented successor those continuation values are reusable for \texttt{Multi} operations and affine, with a runtime reuse guard, for \texttt{Once}. OCaml remains a fine host; the free lunch does not exist.
...
A handler is an interpreter for a set of operations, installed over a region with \texttt{Handle}. Each operation clause binds the operation's arguments and a named resumption: an ordinary reusable function for \texttt{Multi}, or an affine \texttt{Resume} value for \texttt{Once}. Because resumptions are values, control operators that other languages build in (exceptions, generators, async schedulers, backtracking search) are libraries here, and none of them colors the functions above it. The async retrofit damage cataloged in Section~\ref{sec:survey} is the strongest argument for paying the effect system's complexity cost once, up front.
