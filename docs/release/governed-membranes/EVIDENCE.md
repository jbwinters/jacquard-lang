...
| Secret boundary | Workspace normalizer/summarizer cases and GM.11's non-vacuous fixed-secret run prove that review and Audit artifacts omit secret bytes; GM.18 pins live resolution order and zero access on strict refusal and queue denial. The later `relate --vary secret` lane adds bounded program-specific evidence. | Opaque `Secret` ends at explicit exposure. The language has no taint tracking or post-exposure exfiltration guarantee; the relational lane does not revise that historical limit. |
