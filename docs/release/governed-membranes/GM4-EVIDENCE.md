...
`test/cli/governance-policy-laws.jqd` exposes eight independent `Prop` values:
...
confidence grid `0.0, 0.25, 0.5, 0.75, 1.0`. Under the default 10,000
exploration budget, `jac test --exhaustive` reports:
...
all eight exhaustive properties render as cached proofs.
...
The mutation is never applied to the committed fixture. The transcript is both
